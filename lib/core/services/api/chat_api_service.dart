...
final class ChatApiHttpException extends HttpException {
  ChatApiHttpException(this.statusCode, String responseBody)
    : responseBody = RequestLogger.redactBody(responseBody),
      super('HTTP $statusCode: ${RequestLogger.redactBody(responseBody)}');
...
class ChatApiService {
  ...
  static bool supportsOpenAIImagesApiRouting(
    ...
    if (_isBaiziConfig(config)) return false;
  ...
  static String _apiModelId(ProviderConfig cfg, String modelId) {
    if (_isBaiziConfig(cfg)) return modelId;
    try {
  ...
  static String _effectiveApiKey(ProviderConfig cfg) {
    if (_isBaiziConfig(cfg)) return cfg.apiKey;
    try {
  ...
  static Map<String, String> _customHeaders(
    ...
    if (!_isBaiziConfig(cfg)) return out;
    return BaiziGateway.mergeRequestHeaders(
  ...
  static Map<String, dynamic> _customBody(ProviderConfig cfg, String modelId) {
    final ov = _modelOverride(cfg, modelId);
    final out = ModelOverridePayloadParser.customBody(ov);
    if (!_isBaiziConfig(cfg)) return out;
    return BaiziGateway.mergeRequestBody(
      requiredBody: const <String, dynamic>{},
      customBody: out,
    );
  }

  static bool _isBaiziConfig(ProviderConfig config) {
    return config.id.trim().toLowerCase() == BaiziGateway.providerId;
  }
  ...
  static Stream<ChatStreamChunk> sendMessageStream({
    ...
    final isBaizi = _isBaiziConfig(config);
    final runtimeConfig = _runtimeConfig(config, modelId);
    if (isBaizi) {
      BaiziGateway.validateRequest(
        apiKey: runtimeConfig.apiKey,
        availableModels: runtimeConfig.models,
        modelId: modelId,
      );
    }
    final kind = ProviderConfig.classify(
      runtimeConfig.id,
      explicitType: runtimeConfig.providerType,
    );
    final effectiveStream = isBaizi ? true : stream;
    final effectiveExtraHeaders = isBaizi
        ? BaiziGateway.mergeRequestHeaders(
            requiredHeaders: const <String, String>{},
            customHeaders: extraHeaders,
          )
        : extraHeaders;
    final effectiveExtraBody = isBaizi
        ? BaiziGateway.mergeRequestBody(
            requiredBody: const <String, dynamic>{},
            customBody: extraBody,
          )
        : extraBody;
    ...
    final useOpenAIImagesApi =
        ...
        _shouldUseOpenAIImagesApi(runtimeConfig, modelId);
    final unicodeSafeMessages = _sanitizeMessages(messages);
    final stripUnsupportedImageInputs =
        !ocrActive &&
        !useOpenAIImagesApi &&
        !_supportsImageInput(runtimeConfig, modelId);
    ...
    final effectiveClient = client ?? _clientFor(runtimeConfig, cancelToken);
    ...
    try {
      if (kind == ProviderKind.openai) {
        ...
        } else if (runtimeConfig.useResponseApi == true) {
        ...
        final isVertex = runtimeConfig.vertexAI == true;
      ...
      if (kind == ProviderKind.openai) {
        ...
        if (resp.statusCode < 200 || resp.statusCode >= 300) {
          ...
          throw HttpException(
            ...
            '${RequestLogger.redactBody(responseText)}',
          );
        }
