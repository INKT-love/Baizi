  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    if (RequestLogger.enabled) {
      RequestLogger.logLine(
        '[REQ $reqId] $method ${RequestLogger.redactText(uri.toString())}',
      );
      ...
      if (bodyBytes.isNotEmpty) {
        ...
        RequestLogger.logLine(
          '[REQ $reqId] body=${RequestLogger.escape(RequestLogger.redactBody(bodyText))}',
        );
      }
    }
    ...
    try {
      ...
      final int? contentLength = (body.contentLength >= 0)
          ? body.contentLength
          : null;
      final responseLogBuffer = RedactingResponseLogBuffer(
        eventStream: (headers['content-type'] ?? '').toLowerCase().contains(
          'text/event-stream',
        ),
      );
      void logResponseText(String text) {
        for (final entry in responseLogBuffer.add(text)) {
          RequestLogger.logLine(
            '[RES $reqId] chunk=${RequestLogger.escape(entry)}',
          );
        }
      }

      void flushResponseLog() {
        for (final entry in responseLogBuffer.close()) {
          RequestLogger.logLine(
            '[RES $reqId] chunk=${RequestLogger.escape(entry)}',
          );
        }
        ...
        body.stream.listen(
          ...
          onError: (e, st) {
            if (RequestLogger.enabled) {
              flushResponseLog();
              RequestLogger.logLine(
                '[RES $reqId] error=${RequestLogger.escape(RequestLogger.redactText(e.toString()))}',
              );
            }
      ...
      final safeError = RequestLogger.redactText(e.toString());
      if (RequestLogger.enabled) {
        RequestLogger.logLine(
          '[RES $reqId] dio_error=${RequestLogger.escape(safeError)}',
        );
      }
      throw http.ClientException(safeError, uri);
    } catch (e) {
      final safeError = RequestLogger.redactText(e.toString());
      if (RequestLogger.enabled) {
        RequestLogger.logLine(
          '[RES $reqId] error=${RequestLogger.escape(safeError)}',
        );
      }
      throw http.ClientException(safeError, uri);
    }
