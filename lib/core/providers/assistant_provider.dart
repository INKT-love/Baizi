...
class AssistantProvider extends ChangeNotifier {
  ...
  AssistantProvider({this.chatService, this.worldBookProvider}) {
    _load();
  }
  ...
  Future<void> _load() async {
    ...
    if (raw != null && raw.isNotEmpty) {
      ...
      for (int i = 0; i < _assistants.length; i++) {
        ...
        var chatModelProvider = a.chatModelProvider;
        if (chatModelProvider != null &&
            chatModelProvider != BaiziGateway.providerId) {
          chatModelProvider = BaiziGateway.providerId;
          assistantChanged = true;
        }
        final customHeaders = <Map<String, String>>[
          for (final entry in a.customHeaders)
            if ((entry['name'] ?? '').trim().isNotEmpty &&
                !BaiziGateway.isProtectedHeader(entry['name'] ?? ''))
              Map<String, String>.from(entry),
        ];
        final customBody = <Map<String, String>>[
          for (final entry in a.customBody)
            if ((entry['key'] ?? '').trim().isNotEmpty &&
                !BaiziGateway.isProtectedBodyField(entry['key'] ?? ''))
              Map<String, String>.from(entry),
        ];
        if (customHeaders.length != a.customHeaders.length ||
            customBody.length != a.customBody.length) {
          assistantChanged = true;
        }
        if (assistantChanged) {
      ...
      if (changed) {
        try {
          ...
          debugPrint(
            '[AssistantProvider] assistant migration persistence failed: '
            '${error.runtimeType}',
          );
  ...
  Future<void> _deleteCharacterCardSource(String? relativePath) async {
    ...
    try {
      ...
      final file = File(p.joinAll(<String>[directory.path, ...segments]));
      final target = p.normalize(file.absolute.path);
      if (!p.isWithin(root, target)) return;
      if (await file.exists()) await file.delete();

      var parent = file.parent;
      while (p.isWithin(root, p.normalize(parent.absolute.path))) {
        if (!await parent.exists() || !await parent.list().isEmpty) break;
  ...
  /// Persists a fully constructed character-card assistant as one provider
  /// mutation. If preference storage fails, the in-memory list is restored.
  Future<void> commitImportedAssistant(
    Assistant imported, {
    String? replaceAssistantId,
  }) async {
    final snapshot = List<Assistant>.from(_assistants);
    try {
      if (replaceAssistantId == null) {
        if (_assistants.any((assistant) => assistant.id == imported.id)) {
          throw StateError('Assistant ID already exists.');
        }
        _assistants.add(imported);
      } else {
        final index = _assistants.indexWhere(
          (assistant) => assistant.id == replaceAssistantId,
        );
        if (index == -1) {
          throw StateError('Assistant selected for overwrite does not exist.');
        }
        if (imported.id != replaceAssistantId) {
          throw ArgumentError.value(
            imported.id,
            'imported.id',
            'Overwrite must preserve the existing assistant ID.',
          );
        }
        _assistants[index] = imported;
      }
      await _persist();
    } catch (_) {
      _assistants
  ...
  Future<void> updateAssistant(Assistant updated) async {
    ...
    if (next.chatModelProvider != null &&
        next.chatModelProvider != BaiziGateway.providerId) {
      next = next.copyWith(chatModelProvider: BaiziGateway.providerId);
    }
  ...
  Future<bool> deleteAssistant(String id) async {
    ...
    final characterData = removedAssistant.characterData;
    final cardWorldBookId = characterData?.cardWorldBookId;
    if (cardWorldBookId != null && cardWorldBookId.trim().isNotEmpty) {
      try {
        if (worldBookProvider != null) {
          await worldBookProvider!.deleteBook(cardWorldBookId);
        } else {
          await WorldBookStore.delete(cardWorldBookId);
        }
      } catch (_) {}
    }
    await _deleteCharacterCardSource(characterData?.sourceFileName);
    await _deleteManagedFileIfOwned(
      removedAssistant.avatar,
      directoryAsync: AppDirectories.getAvatarsDirectory,
      replacementPath: null,
    );
    notifyListeners();
