...
import 'backup_data_sanitizer.dart';
...
class DataSync {
  ...
  Future<File> prepareBackupFile(WebDavConfig cfg) async {
    ...
    try {
      ...
      final characterCardsDirPath = (await _getCharacterCardsDir()).path;
      ...
      await Isolate.run(() {
        _packZipSync(
          ...
          characterCardsDirPath: characterCardsDirPath,
        );
      });
  ...
  static void _packZipSync({
    ...
    required String characterCardsDirPath,
  }) {
    ...
    try {
      ...
      if (includeFiles) {
        ...
        _addDirectoryToZip(writer, characterCardsDirPath, 'character_cards');
      }
  ...
  Future<Directory> _getCharacterCardsDir() async {
    return await AppDirectories.getCharacterCardsDirectory();
  }
    ...
    try {
      ...
      if (await settingsFile.exists()) {
        try {
          ...
          final map = BackupDataSanitizer.filterSyncPreferences(
          ...
          if (mode == RestoreMode.overwrite) {
            ...
            const credentialMapKeys = {'webdav_config_v1', 's3_config_v1'};
            ...
            for (final entry in map.entries) {
              ...
              if (existing.containsKey(key) &&
                  (credentialListKeys.contains(key) ||
                      credentialMapKeys.contains(key))) {
                try {
                  final merged = credentialListKeys.contains(key)
                      ? _mergeJsonListByIdPreferIncoming(
                          existing[key] as String,
                          newValue as String,
                        )
                      : _mergeJsonMapPreferIncoming(
                          existing[key] as String,
                          newValue as String,
                        );
                  await prefs.restoreSingle(key, merged);
                } catch (_) {}
                continue;
              }
              ...
              if (mergeableKeys.contains(key)) {
                ...
                } else if (key == 'world_books_active_ids_by_assistant_v1' &&
                    existing.containsKey(key)) {
                  try {
                    await prefs.restoreSingle(
                      key,
                ...
                } else if (key == 'world_books_collapsed_v1' &&
                    existing.containsKey(key)) {
                  try {
                    await prefs.restoreSingle(
                      key,
                      _mergeJsonMapPreferExisting(
                        existing[key] as String,
                        newValue as String,
                      ),
                    );
      ...
      if (cfg.includeFiles) {
        if (mode == RestoreMode.overwrite) {
          ...
          // Restore imported character card source files
          final characterCardsSrc = Directory(
            p.join(extractDir.path, 'character_cards'),
          );
          if (await characterCardsSrc.exists()) {
            final dst = await _getCharacterCardsDir();
            if (await dst.exists()) {
              try {
                await dst.delete(recursive: true);
              } catch (_) {}
            ...
            await dst.create(recursive: true);
            for (final ent in characterCardsSrc.listSync(recursive: true)) {
              if (ent is File) {
                final rel = p.relative(ent.path, from: characterCardsSrc.path);
                final target = File(p.join(dst.path, rel));
                await target.parent.create(recursive: true);
                await ent.copy(target.path);
                try {
          ...
          // Merge imported character card source files
          final characterCardsSrc = Directory(
            p.join(extractDir.path, 'character_cards'),
          );
          if (await characterCardsSrc.exists()) {
  ...
  static String _mergeJsonListByIdPreferIncoming(
    String existingRaw,
    String incomingRaw,
  ) {
    final existingList = jsonDecode(existingRaw) as List;
    ...
    void addIncoming(dynamic item) {
      ...
      if (existing == null) {
        ...
        byId[id] = <String, dynamic>{...existing, ...incoming};
      }
    }
  ...
  static String _mergeJsonMapPreferIncoming(
    String existingRaw,
    String incomingRaw,
  ) {
    final existingMap = jsonDecode(existingRaw) as Map;
    final incomingMap = jsonDecode(incomingRaw) as Map;
  ...
  static String _mergeStringListMap(String existingRaw, String incomingRaw) {
    final existingMap = jsonDecode(existingRaw) as Map;
    final incomingMap = jsonDecode(incomingRaw) as Map;
    final merged = <String, Set<String>>{};

    void addAll(Map source) {
      for (final entry in source.entries) {
        final key = entry.key.toString().trim();
        final value = entry.value;
        if (key.isEmpty || value is! List) continue;
        final target = merged.putIfAbsent(key, () => <String>{});
    ...
    addAll(existingMap);
    addAll(incomingMap);
    return jsonEncode(<String, List<String>>{
      for (final entry in merged.entries)
        entry.key: entry.value.toList(growable: false),
    });
  }

  static String _mergeJsonMapPreferExisting(
    String existingRaw,
    String incomingRaw,
  ) {
    final existingMap = jsonDecode(existingRaw) as Map;
...
class SharedPreferencesAsync {
  ...
  Future<Map<String, dynamic>> snapshot() async {
    ...
    for (final k in keys) {
      if (_localOnlyKeys.contains(k) ||
          !BackupDataSanitizer.canSyncPreference(k)) {
        continue;
      }
      map[k] = BackupDataSanitizer.sanitizePreferenceValue(k, prefs.get(k));
    }
    return map;
  }
  ...
  Future<void> restore(Map<String, dynamic> data) async {
    ...
    for (final entry in data.entries) {
      ...
      final v = BackupDataSanitizer.mergePreferenceForRestore(
        k,
        entry.value,
        prefs.get(k),
      );
  ...
  Future<void> restoreSingle(String key, dynamic value) async {
    if (_localOnlyKeys.contains(key) ||
        !BackupDataSanitizer.canSyncPreference(key)) {
      return;
    }
    final prefs = await SharedPreferences.getInstance();
    value = BackupDataSanitizer.mergePreferenceForRestore(
      key,
      value,
      prefs.get(key),
    );
