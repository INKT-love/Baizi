...
abstract class BuiltInToolNames {
  ...
  static String effectiveModelId({
    ...
    if (cfg.id.trim().toLowerCase() == BaiziGateway.providerId) {
      return fallback;
    }
...
abstract class BuiltInToolsHelper {
  ...
  static ProviderKind _providerKind(ProviderConfig config, String modelId) {
    if (config.id.trim().toLowerCase() == BaiziGateway.providerId) {
      return BaiziGateway.protocolForModel(modelId) ==
  ...
  static bool supportsBuiltInSearchForModel({
    ...
    final kind = _providerKind(cfg, modelId!);
