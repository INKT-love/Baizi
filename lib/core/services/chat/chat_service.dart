...
class ChatService extends ChangeNotifier {
  ...
  static const String _characterGreetingGroupPrefix = 'character-greeting:';
  ...
  final Map<String, Future<ChatMessage?>>
  _characterGreetingInitializationOperations = <String, Future<ChatMessage?>>{};
  ...
  Future<ChatMessage?> initializeCharacterGreeting({
    required String conversationId,
    required String firstMessage,
    Iterable<String> alternateGreetings = const <String>[],
  }) async {
    final pending = _characterGreetingInitializationOperations[conversationId];
    if (pending != null) return pending;

    final operation = _initializeCharacterGreeting(
      conversationId: conversationId,
      firstMessage: firstMessage,
      alternateGreetings: alternateGreetings,
    );
    _characterGreetingInitializationOperations[conversationId] = operation;
    try {
      return await operation;
    } finally {
      if (identical(
        _characterGreetingInitializationOperations[conversationId],
        operation,
      )) {
        _characterGreetingInitializationOperations.remove(conversationId);
      }
    }
  }

  Future<ChatMessage?> _initializeCharacterGreeting({
    required String conversationId,
    required String firstMessage,
    required Iterable<String> alternateGreetings,
  }) async {
    if (!_initialized) await init();

    final conversation = getConversation(conversationId);
    final primaryContent = firstMessage.trim();
    if (conversation == null || primaryContent.isEmpty) return null;

    final groupId = _characterGreetingGroupId(conversationId);
    ChatMessage? existingGreeting;
    for (final messageId in conversation.messageIds) {
      final message = _messageForConversation(conversationId, messageId);
      if (message?.groupId != groupId) continue;
      if (existingGreeting == null ||
          message!.version < existingGreeting.version) {
        existingGreeting = message;
      }
    }
    if (existingGreeting != null) return existingGreeting;

    final greeting = await addMessage(
      conversationId: conversationId,
      role: 'assistant',
    ...
    for (final alternateGreeting in alternateGreetings) {
      final content = alternateGreeting.trim();
      if (content.isEmpty) continue;
      final version = await appendMessageVersion(
        messageId: greeting.id,
        content: content,
      );
      if (version == null) {
        throw StateError('Failed to append a character greeting version.');
      }
    }
    await setSelectedVersion(conversationId, groupId, 0);
    return greeting;
  }

  bool hasOnlyCharacterGreetingMessages(String conversationId) {
    final conversation = getConversation(conversationId);
    if (conversation == null) return false;
    if (conversation.messageIds.isEmpty) return true;

    final groupId = _characterGreetingGroupId(conversationId);
    for (final messageId in conversation.messageIds) {
      final message = _messageForConversation(conversationId, messageId);
      if (message == null || message.groupId != groupId) return false;
    }
    return true;
  }

  String _characterGreetingGroupId(String conversationId) =>
      '$_characterGreetingGroupPrefix$conversationId';
  ...
  Future<ChatMessage?> appendMessageVersion({
    ...
    if (!isTemporaryConversation(cid)) {
