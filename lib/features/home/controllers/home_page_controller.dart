...
import 'generation_error_recovery.dart';
...
class HomePageController extends ChangeNotifier {
  ...
  bool get canToggleTemporaryConversation {
    ...
    return conversation != null &&
        _chatService.hasOnlyCharacterGreetingMessages(conversation.id);
  }
  ...
  void _wireViewModelCallbacks() {
    _viewModel.onError = (error) {
      _showGenerationError(error);
    };
    _viewModel.onWarning = (warning) {
      if (warning == 'no_model') {
        _showGenerationError(warning);
      }
    };
  ...
  void _showGenerationError(Object error) {
    ...
    final recovery = classifyGenerationError(error);
    String message;
    String? actionLabel;
    VoidCallback? onAction;
    var type = NotificationType.error;
    switch (recovery) {
      case GenerationErrorRecovery.invalidApiKey:
        message = l10n.baiziInvalidKey;
        actionLabel = l10n.baiziChangeKey;
        onAction = () => unawaited(_openBaiziKeySetup());
      case GenerationErrorRecovery.forbiddenApiKey:
        message = l10n.baiziForbiddenKey;
        actionLabel = l10n.baiziChangeKey;
        onAction = () => unawaited(_openBaiziKeySetup());
      case GenerationErrorRecovery.chooseModel:
        message = error == 'no_model'
            ? l10n.homePagePleaseSelectModel
            : l10n.baiziSelectedModelUnavailable;
        actionLabel = l10n.baiziChooseModelTitle;
        onAction = () => unawaited(_chooseBaiziModel());
        type = NotificationType.warning;
      case GenerationErrorRecovery.none:
  ...
  Future<void> _openBaiziKeySetup() async {
  ...
  Future<void> _chooseBaiziModel() async {
    if (!_context.mounted) return;
    final settings = _context.read<SettingsProvider>();
    try {
      await settings.refreshBaiziModels();
    } on ModelCatalogException catch (error) {
      if (_context.mounted) _showModelRefreshError(error);
      return;
    } catch (_) {
      if (_context.mounted) {
        showAppSnackBar(
          _context,
          message: AppLocalizations.of(_context)!.baiziModelsLoadFailed,
    ...
    final selection = await showModelSelector(
      ...
      initialModelId: recoveryModelSelectionInitialId(settings, assistants),
    );
  ...
  void _showModelRefreshError(ModelCatalogException error) {
    final l10n = AppLocalizations.of(_context)!;
    ...
    final needsKey =
        error.type == ModelCatalogFailureType.unauthorized ||
        error.type == ModelCatalogFailureType.forbidden;
    showAppSnackBar(
      _context,
      message: message,
      type: NotificationType.error,
      duration: needsKey
          ? const Duration(seconds: 6)
          : const Duration(seconds: 3),
      actionLabel: needsKey ? l10n.baiziChangeKey : null,
      onAction: needsKey ? () => unawaited(_openBaiziKeySetup()) : null,
    );
  }

  String _localizeGenerationError(AppLocalizations l10n, Object error) {
    switch (error) {
  ...
  Future<void> translateMessage(ChatMessage message) async {
    ...
    if (result.type == TranslationResultType.error) {
      ...
      if (error != null) {
        final recovery = classifyGenerationError(error);
        if (recovery == GenerationErrorRecovery.invalidApiKey ||
            recovery == GenerationErrorRecovery.forbiddenApiKey) {
          _showGenerationError(error);
          return;
        }
      }
