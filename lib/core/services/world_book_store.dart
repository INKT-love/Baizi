...
class WorldBookStore {
  ...
  static Future<void> save(List<WorldBook> items) async {
    ...
    final persisted = await prefs.setString(_itemsKey, json);
    if (!persisted) {
      throw StateError('World book persistence returned false.');
    }
  ...
  static Future<void> _persistActiveIdsMap(
    ...
    if (!persisted) {
      throw StateError('Active world book persistence returned false.');
    }
  ...
  static Future<void> _persistCollapsedBooksMap(Map<String, bool> map) async {
    ...
    if (!persisted) {
      throw StateError('Collapsed world book persistence returned false.');
    }
