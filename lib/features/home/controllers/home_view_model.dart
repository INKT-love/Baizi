...
class HomeViewModel extends ChangeNotifier {
  ...
  Future<bool> _sendMessageToConversation(
    ...
    if (!result.success) {
      ...
      } else if (result.error != 'empty_input') {
        onError?.call(result.error ?? 'unknown_error');
      }
      return false;
    }
  ...
  Future<void> _initializeCharacterGreeting(
    Conversation conversation,
    Assistant? assistant,
  ) async {
    final characterData = assistant?.characterData;
    if (characterData == null || characterData.firstMes.trim().isEmpty) return;

    final rawUserNickname = _contextProvider.read<UserProvider>().name.trim();
    final userNickname = rawUserNickname.isEmpty
        ? AppLocalizations.of(_contextProvider)!.userProviderDefaultUserName
        : rawUserNickname;
    ...
    await _chatService.initializeCharacterGreeting(
      conversationId: conversation.id,
      firstMessage: resolvePlaceholders(characterData.firstMes),
      alternateGreetings: characterData.alternateGreetings.map(
        resolvePlaceholders,
      ),
    );
  }
  ...
  Future<void> createNewConversation() async {
    ...
    await _initializeCharacterGreeting(conversation, a);
  ...
  Future<void> toggleTemporaryConversation() async {
    ...
    if (convo == null ||
        !_chatService.hasOnlyCharacterGreetingMessages(convo.id)) {
      return;
    }
    ...
    await _initializeCharacterGreeting(conversation, assistant);
