...
class MessageBuilderService {
  ...
  void injectSystemPrompt(
    ...
    if (assistant != null) {
      final userNickname = contextProvider.read<UserProvider>().name;
      final rawSystem = PromptTransformer.buildCharacterCardSystemPrompt(
        assistant,
        userNickname: userNickname,
      );
      if (rawSystem.trim().isEmpty) return;
      ...
      final sys = PromptTransformer.replacePlaceholders(rawSystem, vars);
      apiMessages.insert(0, {'role': 'system', 'content': sys});
    }
  }

  void injectCharacterPostHistoryInstructions(
    List<Map<String, dynamic>> apiMessages,
    Assistant? assistant,
    String modelId,
  ) {
    ...
    final userNickname = contextProvider.read<UserProvider>().name;
    final raw = PromptTransformer.buildCharacterCardPostHistoryInstructions(
      assistant,
      userNickname: userNickname,
    );
    if (raw.isEmpty) return;
...
''');
  ...
  Future<void> injectWorldBookPrompts(
    ...
    final configuredUserNickname = contextProvider
        .read<UserProvider>()
        .name
        .trim();
    var userNickname = configuredUserNickname;
    if (userNickname.isEmpty) {
      if (!contextProvider.mounted) {
        throw StateError(
          'Cannot resolve the default user name from an unmounted context.',
        );
      }
      final localizations = AppLocalizations.of(contextProvider);
      if (localizations == null) {
        throw StateError(
          'Cannot resolve the default user name without localizations.',
        );
      }
      userNickname = localizations.userProviderDefaultUserName.trim();
      if (userNickname.isEmpty) {
        throw StateError('The localized default user name cannot be empty.');
      }
    }
    ...
    try {
      ...
      bool matchesAny(
        ...
        for (final raw in keywords) {
          ...
          if (useRegex) {
            try {
              final re = RegExp(keyword, caseSensitive: caseSensitive);
      ...
      bool isTriggered(WorldBookEntry entry, String context) {
        ...
        final primaryMatched = matchesAny(
          entry.keywords,
          context,
        ...
        if (entry.secondaryKeywords.isEmpty) return false;
        return matchesAny(
      ...
      String resolveContent(WorldBookEntry entry) {
        return PromptTransformer.replaceCharacterCardPlaceholders(
          entry.content,
      ...
      String joinContents(Iterable<WorldBookEntry> items) {
        return items.map(resolveContent).where((c) => c.isNotEmpty).join('\n');
      }
      ...
      List<Map<String, dynamic>> createMergedInjectionMessages(
        ...
        for (final e in injections) {
          if (resolveContent(e).isEmpty) continue;
  ...
  bool hasBuiltInSearch(
    ...
    try {
      final cfg = settings.baiziProviderConfig;
