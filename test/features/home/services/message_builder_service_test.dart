...
class _FixedUserProvider extends UserProvider {
  _FixedUserProvider(this.fixedName);

  final String fixedName;

  @override
  String get name => fixedName;
}

Future<BuildContext> _pumpUserContext(
  WidgetTester tester, {
  ...
  await tester.pumpWidget(
    Localizations(
      ...
      child: ChangeNotifierProvider<UserProvider>.value(
        value: _FixedUserProvider(userName),
        child: Builder(
          builder: (context) {
            result = context;
...
void main() {
  ...
  group('MessageBuilderService.injectWorldBookPrompts', () {
    setUp(() async {
    ...
    testWidgets('replaces character-card placeholders before model injection', (
      tester,
    ) async {
      ...
      final context = await _pumpUserContext(tester, userName: 'Traveler');
      final service = MessageBuilderService(
        chatService: _FakeChatService(const {}),
        contextProvider: context,
      );
      final messages = <Map<String, dynamic>>[
        <String, dynamic>{'role': 'system', 'content': 'Base'},
      ];

      await service.injectWorldBookPrompts(
        messages,
        'assistant-1',
        assistant: const Assistant(id: 'assistant-1', name: 'Luna'),
      );

      expect(messages.single['content'], 'Base\nLuna greets Traveler.');
    });

    testWidgets(
      'replaces case variants and localizes a whitespace user nickname',
      (tester) async {
        const book = WorldBook(
          id: 'book-1',
          entries: <WorldBookEntry>[
        ...
        final context = await _pumpUserContext(tester, userName: ' \t\n');
        final defaultUserName = AppLocalizations.of(
          context,
        )!.userProviderDefaultUserName;
        final service = MessageBuilderService(
          chatService: _FakeChatService(const {}),
          contextProvider: context,
        );
        ...
        expect(
          ...
          'Base\nLuna meets $defaultUserName.',
        );
      },
    );

    testWidgets('requires secondary matches only for selective entries', (
      tester,
    ) async {
      const book = WorldBook(
        id: 'book-1',
        entries: <WorldBookEntry>[
          WorldBookEntry(
            id: 'both',
            keywords: <String>['moon'],
            selective: true,
            secondaryKeywords: <String>['night'],
          ...
          WorldBookEntry(
            ...
            keywords: <String>['moon'],
            selective: true,
            secondaryKeywords: <String>['day'],
            content: 'must-not-inject-missing-secondary',
          ),
          WorldBookEntry(
            id: 'primary-only-mode',
            keywords: <String>['moon'],
            selective: false,
            secondaryKeywords: <String>['day'],
            content: 'primary-only-mode',
          ),
          WorldBookEntry(
            id: 'missing-primary',
            keywords: <String>['sun'],
            selective: true,
            secondaryKeywords: <String>['night'],
            content: 'must-not-inject-missing-primary',
          ),
        ],
      );
      await WorldBookStore.save(const <WorldBook>[book]);
      ...
      final messages = <Map<String, dynamic>>[
        ...
        <String, dynamic>{
          'role': 'user',
          'content': 'The moon is bright at night.',
        },
      ];
      ...
      expect(systemContent, contains('primary-and-secondary'));
      expect(systemContent, contains('primary-only-mode'));
      expect(systemContent, isNot(contains('must-not-inject')));
    });
  });
}
