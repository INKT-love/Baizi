...
class SettingsProvider extends ChangeNotifier {
  ...
  static const String _baiziModelsCacheKey = 'baizi_models_cache_v1';
  static const String _baiziRecentModelsKey = 'baizi_recent_models_v1';
  static const int _maxBaiziRecentModels = 8;
  ...
  bool get hasAnyActiveModel => _baiziModels.isNotEmpty;

  late final Future<void> initialization;
  final SecureApiKeyStore _apiKeyStore;
  final ModelCatalogService _modelCatalogService = const ModelCatalogService();
  ...
  bool get hasCompleteBaiziSetup =>
      isLoaded &&
      hasBaiziApiKey &&
      _currentModelId != null &&
      _baiziModels.contains(_currentModelId);

  ProviderConfig get baiziProviderConfig {
    final persisted = _providerConfigs[BaiziGateway.providerId];
    return ProviderConfig(
      id: BaiziGateway.providerId,
      enabled: hasBaiziApiKey,
      name: 'Baizi',
      apiKey: _baiziApiKey ?? '',
      ...
      useResponseApi: true,
      models: _baiziModels,
      modelOverrides: persisted?.modelOverrides ?? const <String, dynamic>{},
      proxyEnabled: false,
      ...
      claudePromptCachingEnabled:
          persisted?.claudePromptCachingEnabled ?? false,
      claudePromptCachingTtl: persisted?.claudePromptCachingTtl,
    );
  }
  ...
  String resolveOpenAIUpstreamModelId(String providerKey, String modelId) {
    ...
    final kind = _providerKindForConfig(providerKey, cfg, modelId);
  ...
  ProviderKind providerKindForModel(String modelId) {
    return BaiziGateway.protocolForModel(modelId) == BaiziApiProtocol.anthropic
        ? ProviderKind.claude
        : ProviderKind.openai;
  }

  ProviderKind _providerKindForConfig(
    String providerKey,
    ProviderConfig config,
    String modelId,
  ) {
    if (providerKey.trim().toLowerCase() == BaiziGateway.providerId ||
        config.id.trim().toLowerCase() == BaiziGateway.providerId) {
      return providerKindForModel(modelId);
    }
    return ProviderConfig.classify(
      config.id,
  ...
  SettingsProvider({SecureApiKeyStore? apiKeyStore})
    : _apiKeyStore = apiKeyStore ?? SecureApiKeyStore() {
    initialization = _load()
        .catchError((Object error, StackTrace stackTrace) {
          _initializationError = error;
          debugPrint('[SettingsProvider] initialization failed: $error');
  ...
  Future<void> retryInitialization() async {
    ...
    try {
      ...
      _initializationError = error;
      debugPrint('[SettingsProvider] initialization retry failed: $error');
      debugPrint('$stackTrace');
    } finally {
      _isLoaded = true;
      notifyListeners();
  ...
  Future<void> _loadBaiziGatewayState(SharedPreferences prefs) async {
    _baiziModels = List<String>.unmodifiable(
      (prefs.getStringList(_baiziModelsCacheKey) ?? const <String>[])
    ...
    _recentBaiziModels = List<String>.unmodifiable(
      (prefs.getStringList(_baiziRecentModelsKey) ?? const <String>[])
          .map((model) => model.trim())
          .where((model) => model.isNotEmpty && _baiziModels.contains(model))
          .toSet()
          .take(_maxBaiziRecentModels)
          .toList(),
    );

    final selected = prefs.getString(_selectedModelKey);
    final selectedProvider = selected?.contains('::') == true
        ? selected!.split('::').first
        : null;
    final selectedConfig = selectedProvider == null
        ? null
        : _providerConfigs[selectedProvider];
    final legacyConfigs = <ProviderConfig>[
      if (selectedConfig != null) selectedConfig,
      ..._providerConfigs.values.where((config) => config != selectedConfig),
    ];
    String? legacyKey;
    for (final config in legacyConfigs) {
      final primary = config.apiKey.trim();
      if (primary.isNotEmpty) {
        legacyKey = primary;
        break;
      }
      for (final candidate in config.apiKeys ?? const <ApiKeyConfig>[]) {
        if (!candidate.isEnabled || candidate.status != ApiKeyStatus.active) {
          continue;
        }
    ...
    final selectedOverrides = selectedConfig?.modelOverrides;
    _providerConfigs = <String, ProviderConfig>{
      BaiziGateway.providerId: ProviderConfig(
        id: BaiziGateway.providerId,
        enabled: hasBaiziApiKey,
        name: 'Baizi',
        apiKey: '',
        ...
        useResponseApi: true,
        models: _baiziModels,
        modelOverrides: selectedOverrides ?? const <String, dynamic>{},
        proxyEnabled: false,
        multiKeyEnabled: false,
        apiKeys: const <ApiKeyConfig>[],
        keyManagement: const KeyManagementConfig(),
    ...
    _providersOrder = const <String>[BaiziGateway.providerId];
    _providerGroups = const <ProviderGroup>[];
    _providerGroupMap = const <String, String>{};
    ...
    if (secureStorageReady) {
      ...
      await prefs.remove(_providerGroupsKey);
      await prefs.remove(_providerGroupMapKey);
      await prefs.remove(_providerGroupCollapsedKey);
    }
  }

  Future<void> _persistSanitizedProviderConfigs(
    SharedPreferences prefs, {
    Map<String, ProviderConfig>? configs,
  }) async {
    final sanitized = _sanitizedProviderConfigsMap(configs ?? _providerConfigs);
    await prefs.setString(_providerConfigsKey, jsonEncode(sanitized));
  }

  Map<String, dynamic> _sanitizedProviderConfigsMap(
    Map<String, ProviderConfig> source,
  ) {
    return source.map((key, config) {
      final safe = config.copyWith(
        apiKey: '',
        baseUrl: key == BaiziGateway.providerId
            ? BaiziGateway.baseUrl
            : config.baseUrl,
        multiKeyEnabled: false,
        apiKeys: const <ApiKeyConfig>[],
      );
      return MapEntry(key, safe.toJson());
    });
  }

  void _normalizeModelProviderSelections() {
    if (_currentModelId != null) {
      _currentModelProvider = BaiziGateway.providerId;
    }
    if (_titleModelId != null) _titleModelProvider = BaiziGateway.providerId;
    if (_translateModelId != null) {
      _translateModelProvider = BaiziGateway.providerId;
    }
    if (_ocrModelId != null) _ocrModelProvider = BaiziGateway.providerId;
    if (_summaryModelId != null) {
      _summaryModelProvider = BaiziGateway.providerId;
    }
    if (_suggestionModelId != null) {
      _suggestionModelProvider = BaiziGateway.providerId;
    }
    if (_compressModelId != null) {
      _compressModelProvider = BaiziGateway.providerId;
    }
    final normalizedPins = <String>{};
    for (final entry in _pinnedModels) {
      final separator = entry.indexOf('::');
      final modelId = separator < 0 ? entry : entry.substring(separator + 2);
      if (modelId.trim().isNotEmpty) {
        normalizedPins.add('${BaiziGateway.providerId}::${modelId.trim()}');
      }
    }
    _pinnedModels
      ..clear()
      ..addAll(normalizedPins);
  }

  Future<void> _persistNormalizedModelSelections(
    SharedPreferences prefs,
  ) async {
    Future<void> persist(String key, String? modelId) async {
      if (modelId == null || modelId.isEmpty) return;
      await prefs.setString(key, '${BaiziGateway.providerId}::$modelId');
    }

    await persist(_selectedModelKey, _currentModelId);
    await persist(_titleModelKey, _titleModelId);
    await persist(_translateModelKey, _translateModelId);
    await persist(_ocrModelKey, _ocrModelId);
    await persist(_summaryModelKey, _summaryModelId);
    await persist(_suggestionModelKey, _suggestionModelId);
    await persist(_compressModelKey, _compressModelId);
  ...
  Future<_MigrationResult> _migrateEmbeddingModelOverrides(
    ...
    try {
      final encoded = jsonEncode(
        _sanitizedProviderConfigsMap(nextProviderConfigs),
      );
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (providerConfigsLoaded &&
        ...
        if (!prefs.containsKey(_providerConfigsBackupKey)) {
          final backup = _sanitizedProviderConfigsMap(_providerConfigs);
    ...
    // Make ordinary preferences available without waiting for platform secure
    // storage. The setup gate still awaits [initialization] before LLM use.
    notifyListeners();
    await _loadBaiziGatewayState(prefs);

    _normalizeModelProviderSelections();
    await _persistNormalizedModelSelections(prefs);
  ...
  Future<List<String>> refreshBaiziModels({http.Client? client}) async {
    final apiKey = _baiziApiKey;
    if (apiKey == null || apiKey.isEmpty) {
      throw const ModelCatalogException(ModelCatalogFailureType.unauthorized);
    }
    final models = await _modelCatalogService.fetchModels(
      apiKey: apiKey,
      client: client,
  ...
  void _syncBaiziProviderConfig() {
    ...
    _providerConfigs = <String, ProviderConfig>{BaiziGateway.providerId: safe};
    _providersOrder = const <String>[BaiziGateway.providerId];
  }
  ...
  bool isModelPinned(String providerKey, String modelId) =>
      _pinnedModels.contains('${BaiziGateway.providerId}::$modelId');
  Future<void> togglePinModel(String providerKey, String modelId) async {
    final k = '${BaiziGateway.providerId}::$modelId';
  ...
  Future<void> setCurrentModel(String providerKey, String modelId) async {
    ...
    _recentBaiziModels = List<String>.unmodifiable(
      <String>[
        modelId,
        ..._recentBaiziModels.where((recent) => recent != modelId),
      ].take(_maxBaiziRecentModels),
    );
    notifyListeners();
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
      _selectedModelKey,
      '${BaiziGateway.providerId}::$modelId',
    );
    await prefs.setStringList(_baiziRecentModelsKey, _recentBaiziModels);
  }
...
{content}
  ...
  Future<void> setTitleModel(String providerKey, String modelId) async {
    _titleModelProvider = BaiziGateway.providerId;
...
Do not interpret or translate—only transcribe and describe what is visually present.''';
  ...
  Future<void> setOcrModel(String providerKey, String modelId) async {
    _ocrModelProvider = BaiziGateway.providerId;
    _ocrModelId = modelId;
    notifyListeners();
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_ocrModelKey, '${BaiziGateway.providerId}::$modelId');
  }
